...
def compare_whisper_models(fine_tuned_model_path, original_model_name, test_audio_path, language="zh"):
    """
    比較微調模型和原始模型的辨識結果。
    ...
    fine_tuned_model = WhisperForConditionalGeneration.from_pretrained(fine_tuned_model_path).to(device)
    ...
    original_model = WhisperForConditionalGeneration.from_pretrained(original_model_name).to(device)

    # --- 3. 準備測試音訊 ---
    print(f"\n--- 正在載入並處理測試音訊：{test_audio_path} ---")
    # 使用 datasets.Audio 來載入並重新取樣到 16kHz
    audio_dataset = Audio(sampling_rate=16000)
    audio_input = audio_dataset.decode_example(audio_dataset.encode_example(test_audio_path))
    ...
    # 使用微調模型進行預測
    predicted_ids_fine_tuned = fine_tuned_model.generate(input_features)
    transcription_fine_tuned = fine_tuned_processor.batch_decode(predicted_ids_fine_tuned, skip_special_tokens=True)[0]
    
    # 使用原始模型進行預測
    predicted_ids_original = original_model.generate(input_features)
    transcription_original = original_processor.batch_decode(predicted_ids_original, skip_special_tokens=True)[0]
